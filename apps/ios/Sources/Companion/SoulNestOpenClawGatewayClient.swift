...
final class SoulNestOpenClawGatewayConnection: SoulNestGatewayConnection {
    ...
    func connect(to endpoint: SoulNestGatewayEndpoint) async {
        ...
        if let stableID = parameters.stableID,
           GatewaySettingsStore.loadGatewayRegistry().entries.contains(where: {
               GatewayStableIdentifier.matches($0.stableID, stableID)
           })
        {
            _ = await self.controller.switchToGateway(stableID: stableID)
        } else {
    ...
    func cancelPairing() async {
        // Stop waiting for approval without forgetting the paired gateway or
        // its launch-time auto-connect preference.
